...
function DataProvidersCentralised() {
    const [dataProviders, setDataProviders] = useState([]);

    useEffect(() => {
        const fetchDataProviders = async () => {
            try {
                const response = await API.get('/data_providers_centralised', {
    ...
    return (
        <div>
            <h2>Data Providers (Centralised)</h2>
...
export default DataProvidersCentralised;
